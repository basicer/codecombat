...
module.exports = class SpellView extends CocoView
  ...
  onAllLoaded: =>
    @getThatThereToken(@spell.source).then (token) =>
  ...
  recompile: (cast=true, realTime=false, cinematic=false) ->
    @reallyGetTheSourceOrDontWhatever().then (source) =>
  ...
  getThatThereToken: (source) =>
    headers =  { 'Accept': 'application/json', 'Content-Type': 'application/json' }
    m = document.cookie.match(/JWT=([a-zA-Z0-9.]+)/)

    fetch "http://kodekeeper.chessgears.com", {method: 'POST', mode:'cors', headers:headers, body:JSON.stringify({code: source, language: @spell.language, jwt:(if m then m[1] else null)})}
    .then (x) => 
      x.json()
    .then (x) => 
      console.log(x)
  ...
  reallyGetTheSourceOrDontWhatever: () =>
    source = @ace.getValue()
    @getThatThereToken(source)
  ...
  onNewWorld: (e) ->
    @reallyGetTheSourceOrDontWhatever().then (token) =>
      console.log("I think you are rerunning, you will need the token", token)
